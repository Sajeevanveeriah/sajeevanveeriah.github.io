 ...
 * self-contained first-person explanation: what it does, the mechanism
 * underneath it, why I chose it, how I implemented it, the failure modes I
 * guarded against and how I verified the result. Treatments render inside
 * the TechnicalDepth disclosure on the page where the technique is claimed.
 *
 ...
 * knowledge; claims about my systems are limited to what those systems
 * actually did. No rates, gains, resolutions or results appear unless a
 * record already states them.
 */
...
export const techniques: readonly Technique[] = [
  {
    ...
    paragraphs: [
      ...
      'In my ROS 2 rover I ran A* over the occupancy grid that LiDAR SLAM produced, and I kept planning in its own node so the planner could be exercised, tuned and replaced without touching perception or control. Replanning is part of the design: when the map updates or the goal changes, the planner produces a fresh path from the current estimated pose rather than patching the old one.',
      'The failure modes I guarded against are quiet ones: an inadmissible heuristic still returns paths, just silently suboptimal ones, and an inflation radius that is too small produces paths a real chassis cannot track. I verified planning behaviour through repeated Gazebo simulation runs, inspecting the planned paths against the map in RViz and checking that replanning around newly observed obstacles stayed consistent across reruns.',
    ],
  },
  {
    id: 'kalman-filter',
  ...
  },
  {
    ...
    paragraphs: [
      ...
      'In my rover the EKF fused IMU and wheel odometry into one continuous pose estimate. I guarded the divergence risk structurally: the fused estimate was continuously compared against the LiDAR SLAM pose, so drift between the dead-reckoned estimate and the map-referenced one was visible rather than hidden, and the estimation node could be retuned independently of the rest of the stack.',
  ...
  },
  {
    ...
    paragraphs: [
      ...
      'In my rover LiDAR SLAM produced the occupancy grid that everything downstream consumed: the A* planner searched it and the localisation estimate was referenced to it. I kept SLAM as its own node in the ROS 2 stack, so mapping could be restarted and inspected without restarting planning or control.',
      'I verified mapping through repeated simulation runs, inspecting the grid in RViz for the signature failures, doubled walls, bent geometry and unstable localisation, and checking that maps of the same simulated world stayed consistent across reruns rather than diverging run to run.',
    ],
  },
  {
    id: 'sensor-fusion',
    ...
    paragraphs: [
      ...
      'I validated fusion behaviour in repeated Gazebo runs by comparing fused localisation stability against what odometry alone produced, and by watching for the disagreement signatures, fused pose stepping when a correction lands, or drifting between corrections, that indicate the noise models need rebalancing.',
    ],
  },
  {
    id: 'pid-control',
    ...
    paragraphs: [
      ...
      'I have applied PID in two settings the records evidence: motion control in my ROS 2 rover, tuned against simulated step behaviour, and process control loops in PLC logic delivered professionally for regulated manufacturing clients. My tuning practice is iterative and observational: raise proportional gain until the response is fast but oscillatory, add derivative action to damp it, then add just enough integral action to remove the remaining offset, watching overshoot, rise and settling behaviour on each change.',
      'Verification differs by setting but the logic is the same: in simulation I re-ran manoeuvres and checked the response stayed consistent across runs, and in plant work loop behaviour is proven during commissioning and FAT and SAT execution, where the response to setpoint changes and disturbances is exercised and documented rather than assumed.',
    ],
  },
  {
    ...
    paragraphs: [
      ...
      'I decomposed my rover along the natural seams of the autonomy problem: sensing, LiDAR SLAM, state estimation with Kalman and EKF filtering, A* and Nav2 planning, motion control, simulation and visualisation each ran as separate nodes. The launch structure composed them into one system, which is what makes a stack like this operable: one entry point brings the graph up in order with consistent parameters.',
      'The reason for that decomposition is testability. Because every interface is a published message contract, each node can be run against recorded or simulated inputs on its own: the planner can be exercised against a saved map, the estimator against replayed sensor streams. I tuned and validated each layer independently before running the integrated stack, which is the difference between debugging a node and debugging a robot.',
  ...
  },
  {
    ...
    paragraphs: [
      ...
      'MoveIt 2 is the manipulation counterpart: a motion planning pipeline that holds a planning scene, samples collision-free joint-space paths with sampling-based planners, and solves inverse kinematics to reach Cartesian goals. I hold MoveIt 2 at the level of manipulation basics, and I state it at that level: I have worked with the pipeline and its concepts rather than delivered a manipulation system with it.',
      'In the rover I ran Nav2 alongside my own A* planning work, which let me compare a framework planner against one I had implemented, and I validated navigation behaviour the same way as the rest of the stack: repeated simulated runs, goals reached with obstacle avoidance intact, and path and costmap inspection in RViz.',
    ],
  },
  {
    id: 'can-canoe',
    ...
    paragraphs: [
      ...
      'Working at this level means reading the bus as evidence. A captured trace shows what the vehicle network actually did: which frames appeared, at what rate, with what payloads, and how bus load behaved. That is the difference between reporting that a feature misbehaved and showing the frames that prove it, and it is why I captured bus-level evidence for every observation rather than relying on what the vehicle appeared to do.',
      'In my automotive validation work I used Vector CANoe and CANalyzer to capture and analyse CAN and CAN FD traffic while running feature-vehicle, breadboard and over-the-air regression testing across the T6 Ranger and Everest programmes. What I was validating was feature behaviour against its specification: that features behaved correctly across vehicle variants, running changes and software updates feeding readiness milestones.',
      'The hard part of vehicle-level fault evidence is attribution: separating a genuine feature fault from instrumentation error and from procedure error. I used structured, repeatable drives to make failures reproducible before reporting them, so every defect report stood on a trace someone else could re-derive rather than an impression from the driver seat.',
    ],
  },
  {
    id: 'telemetry-path',
  ...
  },
  {
    ...
    paragraphs: [
      ...
      'I built two evidenced systems this way. My Honours capstone ran real-time acquisition on an ESP32 across IMU, time-of-flight, Hall-effect and magnetometer sensors, designed around deterministic real-time acquisition with analysis kept offline in MATLAB where clinical comparison is easier to audit. At DuxTel I developed ESP32 sensing and telemetry firmware that fed the LoRaWAN uplink path on deployed field hardware. I also work across STM32 as a platform, and I state that as platform capability rather than a delivered STM32 product.',
      'The failure modes are timing failures: jitter in the sample interval, a blocking call sneaking into the acquisition path, or a starved interrupt, and they corrupt data in ways that look like sensor noise. My verification was downstream and honest: the capstone measurements were validated against clinical references for repeatability and comparability, and the telemetry firmware proved itself in the field trial, where a timing fault would have surfaced as gaps or drift in the recorded series.',
    ],
  },
  {
    ...
    paragraphs: [
      ...
      'The raw signal is small and everything couples into it: offset and temperature drift in the element, supply noise, and electromagnetic interference from nearby electronics. Conditioning is what turns it into measurement, ranging the signal to the converter and filtering it so the band of interest, human movement, survives while broadband noise is rejected. On my ataxia device the Hall-effect channel sat alongside IMU, time-of-flight and magnetometer channels deliberately, so movement features were captured redundantly across modalities with different noise behaviour.',
      'Calibration and validation were the point of the project: I validated the captured measurements against clinical references, checking that the motion signals were repeatable across captures and comparable against the reference, which is the standard that matters for an assessment-support instrument. The firmware acquired in real time and the analysis ran offline in MATLAB, keeping the comparison auditable.',
      'The record states its boundary plainly: this was my assessed Honours capstone and a research-support concept, built and validated as a proof-of-concept measurement platform, and the clinical-reference validation is the evidence it stands on.',
    ],
  },
  {
    id: 'pcb-design',
    ...
    paragraphs: [
      ...
      'Electromagnetic compatibility is mostly loop area: every signal returns somewhere, and the smaller the loop between a trace and its return, the less it radiates and the less it receives. A board destined for machinery in a paddock also has mechanical EMC: enclosure, mounting, connector strain and vibration, and I made those field-hardening decisions as part of the design rather than after it.',
      'For my DuxTel agricultural monitoring application I designed a custom board that consolidated the field interfaces in one place: CAN trace capture from the equipment, GPS positioning and environmental sensing integrated on one PCB, which replaced a loose collection of modules with a single deliberate design.',
  ...
  },
  {
    ...
    paragraphs: [
      ...
      'My method at JAG Process Solutions was conversion followed by verification, in that order of trust: I converted the SCADA application content methodically, then verified functional behaviour against the existing validated system rather than trusting the conversion. Where an interface decision was mine to make, I prioritised diagnostics, operator usability and data integrity, because a migration is also the moment those either improve or quietly regress.',
      'The evidence trail was part of the delivery: I executed FAT and SAT activities and produced commissioning, qualification and handover documentation, so the migrated system arrived with the proof of equivalence a regulated site requires, and the work I delivered is documented in the smart-factory record this treatment sits on.',
    ],
  },
  {
    ...
    paragraphs: [
      ...
      'My role differs by stage and I state it precisely. At JAG Process Solutions I executed FAT and SAT activities and produced commissioning, qualification and handover documentation within the GAMP 5 lifecycle that IQ and OQ style records belong to. In manufacturing I worked with ITP-driven inspection and MDR documentation across structural-steel and pressure-vessel fabrication, and in automotive testing I produced the structured, auditable results that regulatory scrutiny demands.',
      'The transferable habit is that verification is designed before it is executed: each stage has defined acceptance criteria, evidence is produced at the moment of test rather than reconstructed afterwards, and a fault found is a record made. That habit is visible across my records, from CAN traces attached to defect reports to qualification documents handed over with a migrated SCADA system.',
    ],
  },
  {
