    ...
    <>
      ...
      <main id="main">
        ...
        <section className="project-section" id="work" aria-labelledby="work-title"><div className="shell">
          <div className="section-heading atlas-heading"><div><p className="eyebrow">Project atlas</p><h2 id="work-title">Systems built, tested and deployed.</h2></div><Link className="text-action" href="/work/">Open the record index</Link></div>
          <div className="atlas-records">{featuredProjects.map((project, index) => <article className="atlas-record" key={project.slug}>
            ...
            <div className="atlas-record-copy"><div className="record-title-row"><p className="record-index">{String(index + 1).padStart(2, '0')}</p><h3>{project.title}</h3></div><dl className="record-facts"><div><dt>Problem</dt><dd>{project.problem}</dd></div><div><dt>System boundary</dt><dd>{project.system}</dd></div><div><dt>Verification</dt><dd>{project.verification}</dd></div><div><dt>Current readiness</dt><dd>{project.readiness}</dd></div></dl><ul className="stack-row" aria-label={`${project.title} technology stack`}>{project.stack.map((item) => <li key={item}>{item}</li>)}</ul><div className="record-actions"><Link className="button button-secondary" href={`/work/${project.slug}/`}>Open engineering record</Link></div></div>
          </article>)}</div>
          <ProjectIndex headingId="further-title-home" />
        </div></section>
