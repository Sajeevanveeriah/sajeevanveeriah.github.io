 ...
 * Concept lab copy.
 *
 * Four deterministic demonstrations written to make the techniques the
 * records name legible. Every string the lab routes, the /lab/ index,
 ...
 * the record that documents where the technique was applied. Prose recast
 * to agentless capability voice on the owner's 7 August 2026 direction,
 * facts unchanged.
 */
...
export const labsIndex = {
  ...
  lede: 'These four demonstrations were written to make the core techniques in the records legible: each one is deterministic, runs entirely in the page and shows its real final state before any script loads.',
  note: 'Each demonstration is seeded, so the same inputs give the same numbers on every visit. The records document where each technique was applied; these pages isolate the technique itself so its behaviour is visible.',
} as const
...
export const labs: readonly LabCopy[] = [
  {
    ...
    paragraphs: [
      'This planner was written so the search itself is visible: every cell A* expands is shown in the order it was expanded, and the committed path renders once the goal closes. Paint obstacles onto the grid, move the start and goal, and the readouts follow: nodes expanded, path cost, path length and whether the heuristic stayed admissible for the movement model.',
      ...
      'In the rover this search ran over the occupancy grid LiDAR SLAM produced, documented in the rover record. This page runs the same algorithm on a grid you control, and it runs entirely in the page.',
    ],
    figureNote:
  ...
  },
  {
    ...
    paragraphs: [
      ...
      'Loops like this were tuned in two settings the records document: motion control in the rover simulation, and process loops in PLC logic delivered for regulated manufacturing clients, documented in the Engineering Mastery Lab and smart-factory records. This page isolates the tuning behaviour itself.',
    ],
    figureNote:
  ...
  },
  {
    ...
    paragraphs: [
      ...
      'In the rover, Kalman and EKF estimation fused odometry and IMU data ahead of planning, documented in the rover record. This page runs the same mathematics on a stream you control.',
    ],
    figureNote:
  ...
  },
  {
    ...
    paragraphs: [
      ...
      'The drift this module demonstrates is the reason the rover ran LiDAR SLAM rather than trusting odometry, documented in the rover record: scan matching and loop closure exist to remove exactly the error this page lets you inject.',
    ],
    figureNote:
...
export const embedCopy = {
  ...
  intro: 'These interactive modules were written to make the techniques this record names legible. Each one is deterministic, runs entirely in the page, and opens as a full lab.',
